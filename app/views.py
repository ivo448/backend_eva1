...
def agregarSolicitud(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if not solicitud.usuario: # Asignar usuario si no se seleccionó
...
def editarMantencion(request, id):
    ...
    # Usamos un template genérico para agregar/editar
    return render(request, 'app/mantencion_form.html', data)
...
def editarPerfil(request, id):
    # Nota: La creación de Perfil debería ser automática al crear un User (con Signals)
    # Esta vista asume que el Perfil ya existe.
...
def editarPrestamo(request, id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            # Lógica para ajustar stock al editar
...
class PrestamoDeleteView(LoginRequiredMixin, BSModalDeleteView):
    ...
    # NOTA: Eliminar un préstamo requiere lógica de negocio (¿se devuelve el stock?)
    # Por ahora, solo lo borra.
